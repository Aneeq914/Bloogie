    ...
    <div className="flex flex-col">
      ...
      <div className="relative overflow-hidden bg-linear-to-b from-brand-50/60 via-white to-white">
        <section className="relative mx-auto flex max-w-3xl flex-col items-center px-6 py-16 text-center md:py-24">
          <div className={`h-8 w-40 rounded-full ${skeletonClass}`} />
          <div className={`mt-6 h-12 w-3/4 ${skeletonClass}`} />
          <div className="mt-6 w-full max-w-2xl space-y-3">
            <div className={`h-4 w-full ${skeletonClass}`} />
            <div className={`h-4 w-11/12 ${skeletonClass}`} />
            <div className={`mx-auto h-4 w-2/3 ${skeletonClass}`} />
          </div>
        </section>
      </div>
      ...
      <div className="min-h-screen px-4 py-12 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="mb-8 flex flex-col gap-2">
            <div className={`h-7 w-40 ${skeletonClass}`} />
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {Array.from({ length: 6 }).map((_, i) => (
              ...
              >
                <div className={`h-48 w-full rounded-none ${skeletonClass}`} />
